...
{
    public class WWLLNFileProcessor
    {
        ...
        /// </summary>
        public WWLLNFileProcessor(string _fulLlsPathFileName, Encoding _encode)
        {
            encode = _encode;
        ...
        public List<LightningStrike_WWLLN> ReturnStrikes(bool skipTheFirstRow = false)
        {
            DateTime dtStart = DateTime.Now;
            string strBuffer;
            GetSumLineNumFromText();
            curRow = 1;
            var StrikesList = new List<LightningStrike_WWLLN>();
            using (StreamReader strReader = new StreamReader(sourceLlsFilePathName, Encoding.UTF8))
            {
                while ((strBuffer = strReader.ReadLine()) != null)
                {
                    ...
                    try
                    {
                        ...
                        var rowProcessor = new WWLLNLlsRowProcessor(strBuffer);
