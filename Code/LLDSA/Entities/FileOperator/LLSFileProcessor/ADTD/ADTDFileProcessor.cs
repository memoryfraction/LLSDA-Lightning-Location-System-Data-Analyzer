...
namespace LLDSA.Entities.FileOperator.LLSFileProcessor.ADTD
{
    public class ADTDFileProcessor
    {
        #region 变量|Variables
        ...
        public string FileNameLlsSrc
        {
            get
            {
                fileNameLlsSrc = Path.GetFileName(sourceLlsFilePathName);
        ...
        /// </summary>
        public ADTDFileProcessor(string _fulLlsPathFileName, Encoding _encode)
        {
            encode = _encode;
            ...
            using (StreamReader strReader = new StreamReader(SourceLlsFilePathName, Encoding.UTF8))
            {
                while ((strBuffer = strReader.ReadLine()) != null)
                {
                    try
                    {
                        ...
                        var rowProcessor = new adtdLlsRowProcessor(strBuffer);
