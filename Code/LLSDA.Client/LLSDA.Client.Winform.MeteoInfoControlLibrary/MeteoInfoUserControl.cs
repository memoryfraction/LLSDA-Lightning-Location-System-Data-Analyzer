...
namespace MeteoInfoControlLibrary
{
    public partial class MeteoInfoUserControl : UserControl
    {
       ...
       /// <param name="isLabeled">是否标注强度</param>
        ...
        {
            LlsFileProcessor llsFileProcessor = new LlsFileProcessor(file,Encoding.UTF8);
            //List<LightningStrikeChina> strikeChinaList = new List<LightningStrikeChina>();   
            stopwatch.Start();
            var strikeChinaList = llsFileProcessor.ReturnStrikesChinaByProcess();
            foreach (var tmp in strikeChinaList)
            {
                var strike_Standard = tmp.ConvertToIStrike_Standard();
                strikeStandard.Add(strike_Standard);
            }
        }
        ...
        private void tabPageSearchCondition_Click(object sender, EventArgs e)
        {
